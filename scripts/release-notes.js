...
async function main() {
    const res = await got("https://api.github.com/repos/ludvigalden/use-cleared-memo/releases", {
        responseType: "json",
    });
    ...
    const targetVersion = /v(.*?-)/.exec(allReleases.filter((e) => /v.*?-/.exec(e.tag_name)).pop().tag_name).pop();
    ...
    Object.keys(lineItems).forEach((header) => {
        releases.forEach((release) => {
            ...
            let headerLastIndex = release.body.slice(headerIndex + 1).findIndex((v) => v.startsWith("###"));
    ...
    // Add compare link
    finalMessage.push(`https://github.com/ludvigalden/use-cleared-memo/compare/${firstVersion}...${lastVersion}`);
